...
*******************************************************************************/
	...
	//  Read Parameter File
        ...
        System.out.println("\nInput Data File Name is: " + args[1]);
		Parameters parmValues = new Parameters(args[0]); //"scheduler.params"
		String inputDataFile = args[1]; //"testdata1"
...
//		r.setSeed(Parameters.seed);
		...
		for (R = 1; R <= Parameters.numRuns; R++){
			...
			////@mylist - print test
            //for (int i=0; i<Parameters.popSize; i++){
			//    for (int j = 0; j < member[i].chromo.size(); j++){
            //        System.out.print("(" + member[i].chromo.get(j).person + "," + member[i].chromo.get(j).timeslot + ") ");
            //    }
            //    System.out.println();
            //}
